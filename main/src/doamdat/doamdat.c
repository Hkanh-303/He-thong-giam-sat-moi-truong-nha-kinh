...
static const char *TAG = "DOAMDAT_CAMBIEN";

esp_err_t doamdat_init(adc_oneshot_unit_handle_t *adc_handle, adc_channel_t channel){
    esp_err_t err =  ESP_OK;

    // Cơ chế singleton: Chỉ bật nguồn adc unit 1 lần nếu nó chưa tồn tại
    if (*adc_handle == NULL){
    ...
    // Cấu hình chi tiết cho channel cụ thể của độ ẩm đất
